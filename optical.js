...
//tb popup메뉴
const siteMap = document.querySelector(".xi-align-right");
const tb_popup = document.querySelector(".tb_popup");
const popup_closeBtn = document.querySelector("#popup_closeBtn");

siteMap.addEventListener("click", function (e) {
  e.preventDefault();
  tb_popup.classList.add("active");
});

popup_closeBtn.addEventListener("click", function (e) {
  e.preventDefault();
  tb_popup.classList.remove("active");
});

tb_popup.addEventListener("click", function (e) {
  if (e.target === tb_popup) {
    tb_popup.classList.remove("active");
  }
});
...
//tb하위메뉴
$(function () {
  $(".popMenu > li").click(function () {
    // 하위 메뉴가 있는 li만 동작
    if ($(this).find(".tb_submenu").length > 0) {
      const isOpen = $(this).hasClass("open");

      // 모두 닫기
      $(".popMenu > li").removeClass("open");
      $(".tb_submenu").stop(true, true).slideUp();

      if (!isOpen) {
        // 현재 메뉴만 열기
        $(this).addClass("open");
        $(this).find(".tb_submenu").stop(true, true).slideDown();
      }
    }
  });
});
